import { SafeAreaView, ScrollView, StyleSheet, Text, View, TouchableOpacity, ActivityIndicator, Alert, Pressable } from 'react-native'
...
export default function EditHolidayYatraScreen() {
        ...
        if (type === "text") {
            ...
            setTour({
                ...
                [name]: tour[name].includes(text)
                    ? tour[name].filter((item: string) => item !== text)
                    : [...tour[name], text]
            });
        }
    };
    ...
    const handleSubmit = async () => {
        try {
            ...
            formData.append("price[forPerson]", Number(tour.priceForSingle || '0'));
            formData.append("price[forCouple]", Number(tour.priceForCouple || '0'));
            formData.append("location", tour.location);
            formData.append("arrivalDate", new Date(tour?.arrivalDate)?.toISOString());
            formData.append("departureDate", new Date(tour?.departureDate)?.toISOString());
            formData.append("lastDateToBook", new Date(tour?.lastDateToBook)?.toISOString());
                                            ...
                                            <View style={styles.inputGroup}>
                                                ...
                                                field.type === "array" ?
                                                    <View>
                                                        ...
                                                        <View>
                                                            <TouchableOpacity style={{ padding: 4, borderWidth: 1, borderColor: Colors.secondary }} onPress={() => setTour({ ...tour, [field.name]: [...tour[field.name], ""] })}><Text>Add</Text></TouchableOpacity>
                                                            <TouchableOpacity style={{ padding: 4, borderWidth: 1, borderColor: Colors.secondary }} onPress={() => setTour({ ...tour, [field.name]: tour[field.name].slice(0, -1) })}><Text>Remove</Text></TouchableOpacity>
                                                        </View>
                                                    </View> :
                                                    field.type === "multi" ?
                                                        <View>
